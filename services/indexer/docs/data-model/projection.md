...
- **Capacity** — active-set membership from the `VolumeCreated` / `VolumeRetired` logs merged in chain order, sampled at UTC day end; `depth → effective_bytes` is a fixed lookup (bucketDepth 16, unencrypted; [`docs/usage.md`](../../../../docs/usage.md) §12), nominal bytes = `(1 << depth) × 4096`.
